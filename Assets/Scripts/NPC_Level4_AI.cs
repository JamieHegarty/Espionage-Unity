...
public class NPC_Level4_AI : MonoBehaviour {
	...
	void FindPlayer () {
		...
		// Detect if player is within the field of view or a certain proximity to the npc
		if(((Vector3.Angle(rayDirection, transform.forward)) < fieldOfView) || (Vector3.Distance (player.transform.position, transform.position)) < 3) { 
		...
		//This allows the NPC to detect a trap and walk away from it instead
		if (Physics.Raycast (transform.position, rayDirection, out hit, 2.5f) && hit.collider.gameObject.tag == "Trap") {
			animator.SetBool (canSeePlayer, false);
			animator.SetBool(canSeeBreadcrumb, false);
		}
	}
